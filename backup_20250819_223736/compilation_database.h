...
private:
    // 路径处理方法
    bool processFilePath(LocalCompileCommand& cmd);
    bool tryAlternativePaths(LocalCompileCommand& cmd, const std::string& original_file);
    
    // 编译器路径验证和修复
    std::string validateCompilerPath(const std::string& original_compiler) const;
